...
/**
 * @file calculator-server.cpp
 *
 * Implementation of the Thrift's 'calculator' service (server side).
 *
...
class CalculatorHandler : public lts::tutorial::calculatorIf {
public:
  CalculatorHandler() = default;

  void name(std::string& _return)
  {
    _return.assign(CALCULATOR_INTERFACE_NAME);
  }

  void version(lts::tutorial::version_struct& _return)
  {
    _return.major = CALCULATOR_INTERFACE_VERSION_MAJOR;
    _return.minor = CALCULATOR_INTERFACE_VERSION_MINOR;
    _return.micro = CALCULATOR_INTERFACE_VERSION_MICRO;
  }
  ...
  int32_t subtract(const int32_t arg1, const int32_t arg2)
  {
    return arg1 - arg2;
  }

  int32_t multiply(const int32_t arg1, const int32_t arg2)
  {
    return arg1 * arg2;
  }

  int32_t divide(const int32_t arg1, const int32_t arg2)
  {
    return arg1 / arg2;
  }
};

class CalculatorCloneFactory : virtual public lts::tutorial::calculatorIfFactory {
 public:
  ~CalculatorCloneFactory() override = default;
  lts::tutorial::calculatorIf* getHandler(const ::apache::thrift::TConnectionInfo& connInfo) override
  {
#if defined(USE_STAPLER_TRANSPORT)
...
\*===========================================================================*/
int main() {
  dbg_at2("Starting 'calculator' server ...\n");

  apache::thrift::server::TThreadedServer server(
    std::make_shared<lts::tutorial::calculatorProcessorFactory>(std::make_shared<CalculatorCloneFactory>()),
#if defined(USE_STAPLER_TRANSPORT)
    std::make_shared<lts::ServerStaplerTransport<NON_BLOCKING_TRANSPORT>>(1),
#else
    std::make_shared<apache::thrift::transport::TServerSocket>(CALCULATOR_CONNECTION_PORT), //port
#endif
